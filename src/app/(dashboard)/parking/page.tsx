...
export default function ParkingPage() {
  ...
  async function checkOut(record: ParkingRecord) {
    ...
    const rem = mins % 60;
    const display =
      hrs > 0 ? (rem > 0 ? `${hrs}h ${rem}m` : `${hrs}h`) : `${mins}m`;
    ...
    if (error) toast.error("Failed to check out");
      ...
      toast.success(`${record.plate_number} checked out — ${display}`);
      fetchRecords();
    }
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
            {activeCount} vehicles currently parked
          </p>
        </div>
            ...
            <table className="table">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map((record) => {
                    const overstay =
                      !record.exit_time && isOverstay(record.entry_time);
                    return (
                      <tr key={record.id}>
                        <td className="font-semibold font-mono text-gray-900 dark:text-white">
                          {record.plate_number}
                        </td>
                        ...
                        <td className="text-sm">
                          <div className="text-gray-700 dark:text-gray-300 font-medium">
                            {format(new Date(record.entry_time), "dd MMM")}
                          </div>
                          <div className="text-green-600 dark:text-green-400 font-mono text-xs">
                            {format(new Date(record.entry_time), "HH:mm")}
                          </div>
                        </td>
                        <td className="text-sm">
                          {record.exit_time ? (
                            <>
                              <div className="text-gray-700 dark:text-gray-300 font-medium">
                                {format(new Date(record.exit_time), "dd MMM")}
                              </div>
                              <div className="text-red-500 dark:text-red-400 font-mono text-xs">
                                {format(new Date(record.exit_time), "HH:mm")}
                              </div>
                            </>
                          ) : (
                        ...
                        <td>
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                              {record.exit_time
                                ? (record.total_duration_display ??
                                  calcDurationDetailed(
                                    record.entry_time,
                                    record.exit_time,
                                  ))
            ...
            <div className="space-y-4">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, parking_slot: e.target.value })
                  }
                />
              </div>
              <div className="form-group">
